...
void ProgrammableInterruptController::Remap(const Uint32 offset_master, const Uint32 offset_slave) {
    Uint8 master_mask = IO::InByte(PIC1_DATA);
    Uint8 slave_mask = IO::InByte(PIC2_DATA);

    master_mask = IO::InByte(PIC1_DATA);                        // save masks
    slave_mask = IO::InByte(PIC2_DATA);

    IO::OutByte(PIC1_COMMAND, ICW1_INIT | ICW1_ICW4);  // starts the initialization sequence (in cascade mode)
    IO::Wait();
    IO::OutByte(PIC2_COMMAND, ICW1_INIT | ICW1_ICW4);
    IO::Wait();
    IO::OutByte(PIC1_DATA, offset_master);                 // ICW2: Master PIC vector offset
    IO::Wait();
    IO::OutByte(PIC2_DATA, offset_slave);                 // ICW2: Slave PIC vector offset
    IO::Wait();
    IO::OutByte(PIC1_DATA, 4);                       // ICW3: tell Master PIC that there is a slave PIC at IRQ2 (0000 0100)
    IO::Wait();
    IO::OutByte(PIC2_DATA, 2);                       // ICW3: tell Slave PIC its cascade identity (0000 0010)
    IO::Wait();
    ...
    IO::OutByte(PIC1_DATA, master_mask);   // restore saved masks.
    IO::OutByte(PIC2_DATA, slave_mask);
}
...
void ProgrammableInterruptController::SetMask(Uint8 irq_line) {
    ...
    value = IO::InByte(port) | (1 << irq_line);
    IO::OutByte(port, value);
}
